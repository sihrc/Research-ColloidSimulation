...
Version 3: Optimization and Hammering out parameters
Chris Lee
April 10, 2013
...
def random_force():
    """Calculates the random force on input Particle in each component [x,y,z]
    ...
    d = random_coeff*ne.evaluate("total/10.0") 
   # print "random",d
    #raw_input()
...
def Eulers():
    ...
    #start = time.clock()
    inter_force()
    #print "inter_force" , time.clock() - start 

    #Checks acceleration due to periodic wall boundaries
    #start = time.clock()
    wall_check()
    #print "wall_check", time.clock() - start 

    #Random Force - Van der Waal
    #start = time.clock()
    random_force()
    #print "random force", time.clock() - start 
    #Euler's Method
